...
[![Total Wine](https://img.shields.io/badge/Total%20Wine-10%2F10%20✓%20严档-success?style=for-the-badge)](stample/totalwine/)
[![Academy](https://img.shields.io/badge/Academy-10%2F10%20✓%20严档%2B-success?style=for-the-badge)](stample/academy/)
...
<td align="center" width="110"><h2>10/10</h2><sub><b>Total Wine ✓</b><br/>(严档)</sub></td>
...
三站镜像结构（2026-05-25 加入 totalwine），每个站点目录含 `px_cookie/` (generator) + `source/` (SDK 锁定) + `sample/` (6 批抓包) + `script/` (8+ 个站点专用工具)。

| 站点 | AppID | TAG | FT | Cookie | TTL | 部署档 | SDK Hash |
|---|---|---|---|---|---|---|---|
| **iFood** | `PXO1GDTa7Q` | `U0MmDhUmOnhXSw==` | `401` | `_px3` | 330 s | 宽档 | `b47a639c…` |
| **Grubhub** | `PXO97ybH4J` | `FmYgK1gdJEAP` | `359` | `_px2` | 500 s | 宽档 | `5e81bffc…` |
| **Total Wine** ⭐ | `PXFF0j69T5` | `CFQ7WU4xIS8MXA==` | `401` | `_px2` | 330 s | **严档** | `9335db02…` |
| **Academy** ⭐⭐ | `PXqqxM841a` | `dgYGCzBjH3pyBg==` | `405` | `_px3` | 330 s | **严档+** | `50debea8…` |

所有常量**直接从真实抓包 POST body 提取**（[`stample/{ifood,grub,totalwine,academy}/sample/`](stample/) 每站 6 批可审计），不是文档记忆。Smoke test 各 21/21（iFood）/ 17/17（Grubhub）/ 22/22（Total Wine）/ 13/13（Academy）全通过。**四站全部常量 + 每站不同的 b64 键映射**见 [`skill/AI_re/references/validated-sites.md`](skill/AI_re/references/validated-sites.md)。

**三档：宽档 / 严档 / 严档+**（2026-06-13 新结论）：
- **严档**（Total Wine）额外查：3-POST 链（seq=2 cookie 确认 beacon）、服务端 HMAC 独立验签、Counter 子字段同步、`state.hid`。见 [`gotchas.md`](skill/AI_re/references/gotchas.md) Bug #15-#18。
- **严档+**（Academy）在此之上把 trust 还绑定到 **mint 的传输真实性**：`/ns` token 是 TLS 指纹化的（node 432 / Chrome 504）、模板**必须真 Chrome CDP 抓**（JSDOM 信任分不够）、counter **全模式校验**（`(0,N)` 非法）、整链走 chrome142 持久 session、出口 IP mint 信誉敏感。见 Bug #19-#23 + [`deployment-tiers.md`](skill/AI_re/references/deployment-tiers.md) 第三档。
...
| 站点 | 验证项 | 测试结果 |
|---|---|---|
| **ifood.com.br** | AppID `PXO1GDTa7Q` · TAG `U0MmDhUmOnhXSw==` · FT `401` · cookie `_px3` (ttl 330) · 2-POST 链 · 宽档 | **10/10 通过** |
| **grubhub.com** | AppID `PXO97ybH4J` · TAG `FmYgK1gdJEAP` · FT `359` · cookie `_px2` (ttl 500) · 2-POST 链 · 宽档 | **10/10 通过** |
| **totalwine.com** ⭐ | AppID `PXFF0j69T5` · TAG `CFQ7WU4xIS8MXA==` · FT `401` · cookie `_px2` (ttl 330) · **3-POST 链** · **严档** | **10/10 通过** (2026-05-25) |
...
| **Total Wine** ⭐ | US 美国住宅 (Bright Data) | `GET totalwine.com/search/all?text=wine` (PX-gated SRP HTML) | ✅ HTTP 200 → 1.3 MB 真实 SRP HTML × 10/10 独立 session 不同 exit IP（严档 Layer 3.5 验证）|
| **Academy** ⭐⭐ | US 住宅（每 cookie 换 IP） | `GET academy.com/c/sports-outdoors` (PX-gated；整链走一条 chrome142 session) | ✅ 无 `/captcha/` 重定向 × **10/10** 干净 IP session（严档+ Layer 3.5；trust 绑定 mint TLS + /ns 指纹 + IP 信誉）|
...
│   ├── grub/                            （结构同 iFood）
│   ├── totalwine/                       ⭐ NEW 2026-05-25 (严档部署案例)
│   │   ├── px_cookie/                    totalwine_px2.js + EV1/EV2/EV3 模板 + smoke 22/22 ✓
│   │   ├── source/                       main.min.js (锁定 SHA 9335db02…)
│   │   ├── sample/                       6 批真实抓包 × 13 文件 (含 EV3 解码)
│   │   ├── script/                       8 个脚本（含 3 个严档独有：diff_ev2/find_hmac/smoke_10x_e2e）
│   │   └── RESEARCH_PURPOSE.md           严档 vs 宽档对照 — 5 个根因有详细记录
│   ├── academy/                         ⭐⭐ NEW 2026-06-13 (严档+ 部署案例)
│   │   ├── px_cookie/                    academy_px3.js + EV1/2/3 模板 ×6 轮换 + smoke 13/13 ✓
│   │   │                                 + session_server.py (chrome142 持久 session 边车) + e2e.py
│   │   ├── source/                       init.js (锁定 SHA 50debea8…)
│   │   ├── sample/                       6 批真 Chrome CDP 抓包 × 13 文件
│   │   └── RESEARCH_PURPOSE.md           严档+ — /ns 的 TLS 指纹 + 真 Chrome 模板 + counter 全模式 + IP-mint 信誉
...
# 4. Total Wine — 产生 _px2 (严档, 3-POST 链)
cd ../../../stample/totalwine/px_cookie
node smoke_test.js          # 自检 22/22 ✓ (含 6 项严档独有校验)
# Generator 需要 US 住宅代理：
$env:HTTPS_PROXY = 'http://<user>-session-<id>:<pwd>@<host>:<port>'
node totalwine_px2.js
# 期望: _px2=… ttl=330, ev1=13, ev2=199, ev3=11, seq2_status=200

# 5. Bundle 路径（按压挑战）— 装油猴脚本
# 浏览器装 Tampermonkey → 加载 bundle/script/userscripts/px_bundle3_auto.user.js
# 访问 https://www.ifood.com.br/ → 触发挑战 → 自动拿 _px3
```

500 ms 完成宽档 PX 握手（2 个 collector POST）；严档 3-POST 链约 6 秒。3 个站点共享 10 个加密算法，只有协议组装层不同。
...
**对于逆向问题、安全测试合作或新平台研究意向**：可以提 GitHub Issue，或通过 [GitHub 个人主页](https://github.com/warterbili)上的邮箱直接联系作者。欢迎讨论方法论移植到其它 PX 站点、联合研究、付费渗透测试等合作方向。
