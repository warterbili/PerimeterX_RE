...
所有常量**直接从真实抓包 POST body 提取**（[`stample/{ifood,grub,totalwine}/sample/`](stample/) 每站 6 批可审计），不是文档记忆。Smoke test 各 21/21（iFood）/ 17/17（Grubhub）/ 22/22（Total Wine — 含 6 项严档独有校验）全通过。

**严档 vs 宽档**（2026-05-25 新结论）：Total Wine 证明同一 PX SDK 在不同客户那有不同服务端校验强度。严档额外查：3-POST 链（含 seq=2 cookie 确认 beacon）、服务端 HMAC 独立验签、Counter 子字段同步性、`state.hid` 提取。详细对照见 [`skill/AI_re/references/deployment-tiers.md`](skill/AI_re/references/deployment-tiers.md)，4 个严档陷阱见 [`skill/AI_re/references/gotchas.md`](skill/AI_re/references/gotchas.md) Bug #15-#18。
