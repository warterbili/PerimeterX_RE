...
4. offsets = compute_interleave_offsets(b64.length)  // 见 B.1.2
5. for each o in offsets reverse:
       insert_random_char(b64, o)
6. return b64
```

### B.1.2 Interleave offsets 计算

```javascript
function compute_interleave_offsets(len) {
    // 实测 ~ 4 个 offsets, 每个 = (固定 seed 数组 + length) % len
    const seeds = [0x73, 0x21, 0xa3, 0x47];   // 占位，真实见 ../../../../revers/payload.js
    return seeds.map(s => (s + len) % len);
}
```

### B.1.3 PX 自定义 serialize
...
def px_serialize(obj):
    ...
    if isinstance(obj, str):  return json.dumps(obj)   # 默认 JSON 字符串编码
    if isinstance(obj, list):
        return '[' + ','.join(px_serialize(x) for x in obj) + ']'
    if isinstance(obj, dict):
        return '{' + ','.join(f'"{k}":{px_serialize(v)}' for k, v in obj.items()) + '}'
    if isinstance(obj, datetime):
        return f'"{obj.strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3]}"'   # PX 不补零，慎用 strftime
    return "null"
```

⚠️ **关键差异点**：JavaScript `JSON.stringify({a: undefined})` 输出 `'{}'`，PX 输出 `'{"a":"undefined"}'`。

---

## B.2 pc — HMAC-MD5 + digit 提取
...
def decode_ob(ob_str, tag):
    ...
    binary = base64.b64decode(ob_str).decode('latin-1')   # ⚠️ 不是 utf-8
    xored = ''.join(chr(ord(c) ^ xor_key) for c in binary)
    segments = xored.split('~~~~')                          # ⚠️ 4 个 ~ 不是 3 个
...
不能按 handler_key 名识别（每次混淆都变），按 args shape：
...
完整 27 个 handler 见 [`../../../../skill/AI_re/references/handler-table.md`](../../../../skill/AI_re/references/handler-table.md)。

---

## B.4 sid — Plane 14 Unicode Tag 隐写
...
def decode_sid(s):
    ...
    return '-'.join([''.join(out[:8]), ''.join(out[8:12]), ...])   # 重组 UUID 格式
```
...
def uuid_v1():
    ...
    # 7. clockseq = random 14-bit + variant bits (PX 不用 system clock)
    # 8. node = random 48-bit (PX 不用 MAC)
...
⚠️ Python 标准库 `uuid.uuid1()` 用真实 MAC + 系统时钟，**会泄漏机器信息**。PX 用随机 clockseq + 随机 node，必须自己实现。

---

## B.6 hash — djb2 + ml

### B.6.1 ml() — 字符串到 32-bit signed
...
## B.7 memory — performance.memory 合成
...
def synth_memory():
    return {
        ...
        'jsHeapSizeLimit': 4294967296,   # 2^32, 所有 Chrome 都是这个
    }
```
...
```python
def te(seed, n):
    """seed 是 state.to, n 是 state.no % 10 + offset"""
    out = []
...
⚠️ `SOME_FACTOR` 和 offset (+1/+2) 跨小版本可能微调，每次升级看 [`../../../../revers/antitamper.js`](../../../../revers/antitamper.js) 最新实现。
...
⚠️ 返回值可能是 `{sm: null, duration: 0}`，**这是正常的**（坑 #25）。
...
下一节：[Appendix C: 10 条避坑必读](C_avoid_traps.md)。
