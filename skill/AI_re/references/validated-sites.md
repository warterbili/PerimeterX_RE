...
> 5 个跑通的真实站点的**全部具体常量 + 每站不同的 b64 键映射 + 部署档位**。
...
| **Walmart** ⚠️ | 宽档 | `_px3` (TTL 330) | 2 (seq 0/1) | ✗ | ✗ 不取 | ✗ | 无字段 | node TLS 即可（US 出口 IP）|
...
> ⚠️ **Walmart = PX-layer-only 案例。** 站点主防护是 **Akamai Bot Manager**，PX 只是次级层。
> `_px3` 单独**不能**过业务 API（Akamai `_abck`/`bm_sv` 才是主门，超本 skill 范围）。
> 收录它是因为它是一个干净的**宽档 + 跨事件一致性**参考（见文末 Walmart 注记）。
...
| **Walmart** | `OkFBAHwnTTY=`=HMAC(uuid,UA) | `GmEhYFwIKVQ=`=HMAC(**state.vid**,UA) | `MklJCHQkQjs=`=HMAC(**state.pxsid**,UA) | `CzBwcU5bfEI=`=**MD5(state.vid)** 单参 |
...
## Walmart note (PX-layer-only lenient + cross-event-consistency reference)

> **EN**: the column tables above (constants / state.* / wire char) are 4-site wide;
> Walmart's values are collected here. Full snapshot in
> [`stample/walmart/source/SDK_INFO_EN.md`](../../../stample/walmart/source/SDK_INFO_EN.md).
> Walmart is **PX-layer only** — the enforcing gate is **Akamai** (out of scope); the
> `_px3` is accepted by the PX collector (10/10) but does not pass business APIs.
>
> **ZH**: 上面的列式表（常量 / state.* / wire char）是 4 站结构；Walmart 的对应值汇总在此，
> 完整快照见 [`stample/walmart/source/SDK_INFO.md`](../../../stample/walmart/source/SDK_INFO.md)。

| 项 | 值 |
|---|---|
| AppID | `PXu6b0qd2S`（稳定）|
| TAG / FT | `eW5CaD8AUB99Zg==` / `396`（运行时从 SDK 拉，轮换）|
| Collector | `collector-pxu6b0qd2s.px-cloud.net/api/v2/collector`（三方）|
| SDK | 一方 `walmart.com/px/PXu6b0qd2S/init.js` + 三方 `client.px-cloud.net/PXu6b0qd2S/main.min.js` |
| Cookie | `_px3`（hex，TTL 330）|
| EV1 / EV2 event-type `t` | `eW5CLzwARxg=` / `bRJWEyt5UyE=` |
| OB XOR key `ml(TAG)%128` | `41` |
| state.no / to / eo / appId | `HwRkBVluazY=`(**parseInt**) / `UBdrVhZ+Z2U=` / `EFcrFlU9JSQ=` / `LDNXMmlcWgg=` |
| anti-tamper | 1 槽，`key=te(val,1)`，`val` 为 20 字符 `[0x30-0x3f]` 随机，**与 wire-state 无关**（不像 totalwine 的 `te(state.to,no%10+1)`）|

⭐ **Walmart's lesson: cross-event consistency** (`cross_event_consistency.py`). Across
EV1↔EV2: `ajERMCxcFQc=` (page-load epoch) + `HUJmQ1soY3c=` (uuid) must be **CONSTANT**;
`Ahk5WERyM2o=` / `U0goSRYkLHs=` / `SlFxEA86eyc=` must be **MONOTONIC** (EV2≥EV1). The old
generator used independent per-event randoms → lenient server still passed, but the wire
was not faithful. **Run this tool on any multi-POST chain** to upgrade "single snapshot
correct" to "cross-event correct". The long token `AEc7BkUsMTA=` is standard base64 (not
url-safe), length 500-512.

⭐ **Walmart 教的一课：跨事件一致性**（`cross_event_consistency.py`）。EV1↔EV2 间：
`ajERMCxcFQc=`（页面加载 epoch）+ `HUJmQ1soY3c=`（uuid）必须 **CONSTANT**；
`Ahk5WERyM2o=` / `U0goSRYkLHs=` / `SlFxEA86eyc=` 必须 **MONOTONIC**（EV2≥EV1）。
旧 generator 每事件独立随机 → 宽档放行但 wire 不真。**任何站点的多 POST 链都该跑一遍这个工具**，
把"单张快照对了"升级到"跨事件也对"。长 token `AEc7BkUsMTA=` 是标准 base64（非 url-safe）、长 500-512。
