#!/usr/bin/env python3
"""
find_state_keys.py — iFood Stage 5 关键脚本
...
STATE_VARS = ["no", "to", "qa", "vid", "pxsid", "appId", "cts", "o111val"]
...
for var in STATE_VARS:
    # 在每批样本中找候选 key（值等于 state[var] 的 EV2 key）
    candidate_sets = []
    for b in batches:
        v = b["state"].get(var)
        if v is None:
    ...
    if confirmed:
        ...
        # 没有跨 6 批一致命中 — 可能是数值类型差异，再试 str(v) == str(ev2_v)
        mapping[f"state.{var}"] = {
            "ev2_b64_key": None,
            "candidates_per_batch": [len(s) for s in candidate_sets],
...
# 打印 + 写文件
print("═══ iFood state.* → EV2 b64 key 映射 ═══")
