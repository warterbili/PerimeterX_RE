#!/usr/bin/env node
/**
 * Build EV1 / EV2 templates for totalwine from the 6 captured batches.
 *
 * Outputs:
 *   tmp/totalwine/px_cookie/totalwine_ev1_template.json
 *   tmp/totalwine/px_cookie/totalwine_ev2_template.json
 *   tmp/totalwine/px_cookie/totalwine_ev1_field_map.json
 *   tmp/totalwine/px_cookie/totalwine_ev2_field_map.json
 *
...
function buildTable(evNum) {
    ...
    fs.writeFileSync(
        path.join(OUT, `totalwine_ev${evNum}_template.json`),
        JSON.stringify([template], null, 2));
    fs.writeFileSync(
        path.join(OUT, `totalwine_ev${evNum}_field_map.json`),
    ...
    dyn.forEach(d => console.log(`  [${d.category.padEnd(11)}] ${d.key}  ←  ${JSON.stringify(d.sample).slice(0,80)}`));
