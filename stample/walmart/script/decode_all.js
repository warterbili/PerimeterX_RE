...
function decodeObSegments(responseJsonPath, tag) {
    const raw = fs.readFileSync(responseJsonPath, 'utf8').trim();
    if (!raw) return null;
    // processOb expects the WHOLE response JSON string + gt (TAG).
    // ⭐ OB XOR key = ml(tag) % 128. The tag ROTATES per SDK instance, so it
    // MUST come from the live request (NEVER hardcode — that was the old bug).
    if (!tag) throw new Error('no tag for OB decode');
    return processOb(raw, decodeURIComponent(tag));
}
...
for (const b of batches) {
    ...
    for (const i of [1, 2, 3]) {
        ...
        if (fs.existsSync(rs)) {
            try {
                const obDec = decodeObSegments(rs, batchTag);
                if (obDec) {
